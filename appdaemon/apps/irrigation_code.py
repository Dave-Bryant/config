...
class Home_Irrigation(hass.Hass):
  ...
  def main_routine(self, *args):
     ...
     if int(self.get_state('sensor.smart_irrigation_hourly_adjusted_run_time')) > 0 and self.running_time > self.watering_threshold and self.chance_of_precipitation < self.precipitation_threshold and self.chance_of_precipitation_48hrs < self.precipitation_threshold:
